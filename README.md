...
- **Traefik basicAuth must not cover `/ws`.** Browsers do not attach cached Basic credentials to a WebSocket handshake, so Open WebUI's socket.io connection gets a 401, retries with backoff, and re-triggers the browser's auth prompt on every attempt — the page appears to ask for credentials endlessly. A separate higher-priority router serves `/ws` without the middleware; Open WebUI still authenticates that path with the user's session token.
